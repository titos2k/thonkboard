...
export function useGraph(boardId: string) {
  ...
  const updateNode = useCallback(
    (id: string, patch: Partial<Pick<ThonkNode, 'title' | 'body' | 'summary' | 'resolved' | 'resolvedAs' | 'conflicts' | 'type' | 'placeholder' | 'thumb' | 'emoji' | 'userTitleEdited' | 'nodeWidth' | 'nodeHeight'>> & { meta?: Partial<ThonkNode['meta']> }) => {
      setGraph(g => ({
        ...g,
        nodes: g.nodes.map(n => {
          if (n.id !== id) return n
