...
interface MergeConflictModalProps {
  ...
  onChoose: (body: string, title?: string) => void
  onCancel: () => void
}

export function MergeConflictModal({ open, conflictDescription, options, onChoose, onCancel }: MergeConflictModalProps) {
  return (
    <Dialog open={open} onOpenChange={v => !v && onCancel()}>
      <DialogContent className="max-w-xl">
        <DialogHeader className="pb-2">
          <DialogTitle>Conflict detected!</DialogTitle>
          <DialogDescription className="text-sm">{conflictDescription}</DialogDescription>
        </DialogHeader>
        ...
        <div className="grid grid-cols-2 gap-3">
          {options.map((opt, i) => (
            <Button
              ...
              onClick={() => onChoose(opt.body, opt.title)}
            >
