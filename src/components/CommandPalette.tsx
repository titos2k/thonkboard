...
const TYPE_ORDER = ['core', 'idea', 'problem', 'question', 'answer'] as const
...
export function CommandPalette({ open, onClose, nodes, onNavigate }: CommandPaletteProps) {
  ...
  const filtered = useMemo(() => {
    ...
    const active   = nodes.filter(n => n.type !== 'note' && !n.resolved && matches(n))
    const resolved = nodes.filter(n => n.type !== 'note' &&  n.resolved && matches(n))
    return [...active, ...resolved]
  }, [nodes, query, typeFilter])

  useEffect(() => {
    if (open) { setQuery(''); setTypeFilter(new Set()); setActiveIdx(0) }
  }, [open])
        ...
        >
          ...
          {presentTypes.length > 1 && (
            <div className="flex items-center gap-1.5 px-4 py-2 border-b border-border">
              ...
              {typeFilter.size > 0 && (
                <button
                  onClick={() => { setTypeFilter(new Set()); setActiveIdx(0) }}
